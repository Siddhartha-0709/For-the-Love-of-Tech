  ...
  const CourseCard = ({ course }) => {
    ...
    >
      ...
      <div className="w-full sm:flex-grow mt-4 sm:mt-0">
        <h5 className="text-xl sm:text-2xl font-bold mb-2 sm:mb-3">{course.courseName}</h5>
        <p className="text-base sm:text-lg">{course.description.substr(0, 910)}{course.description.length > 300 ? '...' : ''}</p>
      </div>
    </div>
  ...
  return (
    <div className="p-5 py-5 bg-black text-white h-full">
      ...
      <div className='h-60 item-center flex justify-center'>
        <h1 className="text-3xl md:text-5xl font-bold text-center mb-4 mt-28 bg-clip-text bg-gradient-to-r from-green-400 to-blue-500 text-transparent" style={{ WebkitTextStroke: '0px black' }}>Upgrade your Tech Potential</h1>
      </div>
